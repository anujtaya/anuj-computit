...
<div class="wrapper">
   ...
   <div id="over_map_bottom" class="text-center">
      ...
      <a class="btn btn-block btn-sm theme-background-color card-1  mt-2"
         href="{{route('service_seeker_home')}}?showBooking=on" onclick="toggle_animation(true);">Switch to Seeker - I
         want work done</a>
      <a class="btn btn-block btn-sm text-white mt-2 card-1" style="background:#5D29BA!important;color:white!important;"
         href="{{route('service_provider_home')}}" onclick="toggle_animation(true);">Switch to Provider - I want to
         work</a>
   </div>
   <div id="over_map_top">
      @if(count($jobs) > 0)
      <div class="bg-white fs--1 shadow-sm theme-color rounded p-3">
         <a href="{{route('service_seeker_jobs')}}" class="text-decoration-none theme-color"
            onclick="toggle_animation(true);">You currently have jobs pending on your job board. Tap here to go to jobs
            tab <i class="fas fa-arrow-right"></i></a>
      </div>
      @endif
      ...
      <div class="bg-white fs--1 shadow-sm theme-color rounded p-3">
         ...
         <a href="{{route('service_seeker_job',$unpaid_job->id)}}" class="btn theme-background-color btn-sm card-1"
            onclick="toggle_animation(true);">Pay ${{number_format($unpaid_job->amount_due,2)}} <i
   ...
   <div class="modal-dialog modal-dialog-centereds" role="document">
      <div class="modal-content border-0 card-1">
         <div class="modal-body text-center" style="min-height:300px;">
            ...
            <input type="text" class="form form-control" id="user_location_modal_manual_popup_input"
