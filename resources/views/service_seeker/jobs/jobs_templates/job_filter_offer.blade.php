<ul id="service_seeker_job_filter_offer_ul" class="list-group">
 @foreach($conversations as $conversation)
 <li class="list-group-item m-1 card-1 fs--1 border-0" onclick="location.href= '{{route('service_seeker_job_conversation', [$conversation->job_id, $conversation->service_provider_id, 'JFO'])}}';toggle_animation(true);" >
	<div class="d-flex bd-highlight mb-2">
		<div class="p-0 mt-1 bd-highlight">
			<img src="{{url('/')}}/storage/images/profile/{{$job->service_seeker_profile->profile_image_path}}"  onerror="this.src='data:image/png;base64,'" class="p-1 border" id="trigger_image" height="40" width="40" style="border-radius:50%;" alt="">
		</div>
		<div class="p-2 bd-highlight">
			<span class="theme-color" style="font-size: 0.8rem;">{{$conversation->first}} {{$conversation->last}}</span> <br>
			<span class="text-warning">   <i class="fas fa-star-half-alt"></i> </span>
		</div>
		<div class="ml-auto p-0 bd-highlight">
			<span class="text-success fs-1">${{$conversation->json['offer']}}</span> <br>
		</div>
	</div>
	<div class="text-muted bg-light p-2 mb-1 rounded">
		{{$conversation->json['offer_description']}}
	</div>
	<span class="text-muted font-weight-normal fs--1 p-1">{{count($conversation->conversation_messages)}} messages</span>
 </li>
 @endforeach
</ul>
