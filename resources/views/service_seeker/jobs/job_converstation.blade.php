@extends('layouts.service_seeker_master')
@section('content')
<script src="{{asset('js/service_seeker/service_seeker_conversation.js')}}?v={{rand(1,100)}}"></script>
<div class="container ">
   <div class="row  justify-content-center" >
      <div class="col-lg-12 shadow-sm sticky-top bg-white p-3 border-d">
         <div class="row">
            <div class="col-2">
               <a href="{{route('service_seeker_job',$job->id)}}" onclick="toggle_animation(true);"><i class="fas fa-arrow-left fs-1" style="color:#399BDB!important"></i> </a>
            </div>
            <div class="col-8 font-size-bolder text-center font-weight-bold theme-color">{{$conversation->service_provider_profile->first}} {{$conversation->service_provider_profile->last}}<br><span class="fs--2 text-muted font-weight-normal"> Active Conversation</span> </div>
            <div class="col-2 text-right">
               <a href="{{route('service_seeker_service_provider_profile',$conversation->service_provider_id)}}" onclick="toggle_animation(true);">
                  <img src="{{url('/')}}/storage/images/profile/{{$conversation->service_provider_profile->profile_image_path}}"  onerror="this.src='data:image/png;base64,'" class="" id="trigger_image" height="30" width="30" style="border-radius:50%;" alt="">
               </a>
            </div>
            <div class="col-12 p-0">
               @if($job->status == 'OPEN')
               <div class="row ml-2 mr-2 mt-3">
                  <div class="col-6 p-1">
                     <button type="button" class="btn btn-block btn-danger fs--1 text-danger-d text-white card-1"  data-target="#jobConversationRejectOfferModal" data-toggle="modal">Reject Offer</button>
                  </div>
                  <div class="col-6 p-1">
                     <button type="button" class="btn btn-block btn-success fs--1 text-success-d text-white card-1" data-toggle="modal" data-target="#jobConversationAcceptOfferModal">Accept Offer</button>
                  </div>
               </div>
               @endif
            </div>
         </div>
      </div>
      <div class="col-lg-12  mt-2  p-0" >
         @include('service_seeker.jobs.job_conversation_partial')
      </div>
   </div>
</div>
@include('service_seeker.jobs.modals.job_conversation_accept_offer_action_modal')
@include('service_seeker.jobs.modals.job_conversation_reject_offer_action_modal')
@endsection