@extends('layouts.service_provider_master')
@section('content')
<script src="{{asset('js/service_provider/service_provider_conversation.js')}}?v={{rand(1,1000)}}"></script>
<div class="container ">
   <div class="row  justify-content-center" >
      <div class="col-lg-12 shadow-sm sticky-top bg-white p-3 border-d">
         <div class="row">
            <div class="col-2"><a href="{{route('service_provider_job',$conversation->job_id)}}" onclick="toggle_animation(true);"><i class="fas fa-arrow-left fs-1" style="color:#5D29BA!important"></i> </a> </div>
            <div class="col-8 font-size-bolder text-center font-weight-bold theme-color">{{$service_seeker_profile->first}} {{$service_seeker_profile->last}}  <br><span class="fs--2 text-muted font-weight-normal"> Active Conversation</span> </div>
            <div class="col-2 text-right">
              <!-- <a href="{{url('/jobs/service_provider/profile/1')}}"> -->
                  <img src="{{url('/')}}/storage/images/profile/{{$service_seeker_profile->profile_image_path}}"  onerror="this.src='data:image/png;base64,'" class="" id="trigger_image" height="30" width="30" style="border-radius:50%;" alt="">
               <!-- </a> -->
            </div>
         </div>
      </div>
      <div class="col-lg-12  mt-2  p-0" >
         @include('service_provider.jobs.partial.job_conversation_partial')
      </div>
   </div>
</div>
@endsection
